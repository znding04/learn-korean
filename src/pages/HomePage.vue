...
const xpToday = computed(() => {
  const lastDate = localStorage.getItem('korean-xp-date')
  if (lastDate !== todayStr()) return 0
  return parseInt(localStorage.getItem('korean-xp-today') || '0', 10)
})
...
onMounted(async () => {
  const vocab = await fetch('/content/topic1-vocab.json').then(r => r.json())
  const due = getDueCards(null, vocab)
...
<template>
  <div class="home">
    <div class="hero">
      <h1 class="hero-title">Learn Korean</h1>
      <p class="hero-tagline">Through Chinese</p>
      ...
      <div class="bridge-callout">
        <span class="bridge-icon">한글</span>
        <p>60% of Korean words come from Chinese characters. If you read Chinese, you already know thousands of Korean words!</p>
      </div>
      ...
      <p class="hero-subtitle">Free, fun, and made for Chinese speakers</p>
      ...
      <div class="hero-links">
        ...
        <a href="https://github.com/znding04/learn-korean" target="_blank" rel="noopener">View source on GitHub → github.com/znding04/learn-korean</a>
      </div>
    </div>
