...
const STORAGE_KEY = 'korean-achievements'
...
const ACHIEVEMENTS = [
  ...
  { id: 'topik1_done', label: 'TOPIK I Graduate', description: 'Complete all TOPIK I lessons', icon: '🎓', condition: (state) => Object.keys(state.lessonComplete).filter(id => Number(id) <= 10).length >= 10 },
]
...
export function useAchievements() {
  ...
  function checkNewAchievements(progressState) {
    const state = {
      ...progressState,
      reviewSessions: parseInt(localStorage.getItem('korean-review-sessions') || '0', 10),
    }
    const newBadges = []
