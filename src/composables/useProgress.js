...
const STORAGE_KEY = 'korean-progress'
...
export function useProgress() {
  ...
  function addXP(amount) {
    ...
    const storedDate = localStorage.getItem('korean-xp-date')
    const storedTodayXP = parseInt(localStorage.getItem('korean-xp-today') || '0', 10)
    if (storedDate === today) {
      localStorage.setItem('korean-xp-today', storedTodayXP + amount)
    } else {
      localStorage.setItem('korean-xp-date', today)
      localStorage.setItem('korean-xp-today', amount)
    }
    saveState(state)
  }
